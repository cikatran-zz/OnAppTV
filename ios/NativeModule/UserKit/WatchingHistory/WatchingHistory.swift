...
public class WatchingHistory: NSObject {
    ...
    var localWatchingHistory = [String: [String: Any]]()
    var ids = [String]()
    var isFetched = false
            ...
            if let result = (result as? [String: Any])?[UserKitKeys.ContinueWatching.rawValue] as? [AnyObject] {
                for i in 0..<result.count {
                    ...
                    if let movieJSON = movie as? [String: Any], let movieId = movieJSON[UserKitKeys.Id.rawValue] as? String {
                        self.isFetched = true
                        self.localWatchingHistory[movieId] = movieJSON
                        self.ids.append(movieId)
                    }
                }
            }
        }, failBlock: { _ in
    ...
    public func remove(id: String, completion: ((Any)-> Void)?,  errorBlock: ((Any)-> Void)?) {
        //localWatchingHistory.removeValue(forKey: id)
//        queue.async {
...
//        }
    ...
    public func sync() {
        ...
        UserKit.mainInstance().profile.set(properties: [UserKitKeys.ContinueWatching.rawValue: self.ids.map{ localWatchingHistory[$0]!}], successBlock: { (result) in
    ...
    public func getConsumedLength(id: String, completion: @escaping (Double) -> Void) {
        if let _ = localWatchingHistory[id] {
            let result = localWatchingHistory[id]![UserKitKeys.ContinueWatching.rawValue] as? Double ?? 0
            completion(result)
            return
        }
        completion(0)
...
//        }
    ...
    public func updateWatchingHistory(id: String, properties: [String: Any], completion: ((Any)-> Void)?, errorBlock: ((Any)-> Void)?) {
        localWatchingHistory[id] = properties
        var newIds = self.ids.filter{ $0 != id }
        newIds.append(id)
        self.ids = newIds
        completion?("")
//        queue.async {
...
//        }
    ...
    public func getWatchingHistory(completion: @escaping ([Any]) -> Void, errorBlock: ((Any)-> Void)?) {
        if (isFetched) {
            let arr = self.ids.map{ localWatchingHistory[$0]!}
            var reversedList = [Any]()
            for arrayIndex in stride(from: arr.count - 1, through: 0, by: -1) {
                reversedList.append(arr[arrayIndex])
            }
